...
public class test {
			...
			int numThread, atomicNodelist list1) throws InterruptedException, ExecutionException
	{
    	...
    	//do{
    		...
    		futurecall =executor.submit(new callTask0(parser, 2));
    		...
    		if(numSubClass != 0)
    		{
    			...
    			{
    				executor.submit(new runTask0(reasoner, manager, list1, groups.get(indexThread2), 2));
    		...
    		futurecall =executor.submit(new callTask0(parser, 3));
    		...
    		if(numEquivalent != 0)
    		{
    			...
    			{
    				executor.submit(new runTask0(reasoner, manager, list1, groups.get(indexThread3), 3));
    		...
    		futurecall =executor.submit(new callTask0(parser, 4));
    		...
    		if(numDisjoint != 0)
    		{
    			...
    			{
    				executor.submit(new runTask0(reasoner, manager, list1, groups.get(indexThread4), 4));
...
///?????????????????  figure out how to shut them down before going on!!
    ...
    {		
    	...
    	//do{
    		...
    		futurecall =executor.submit(new callTask0(parser, 1));
    		...
    		if(numSuperClass != 0)
    		{
    			...
    			{
    				executor.submit(new runTask0(reasoner, manager, list1, groups.get(indexThread1), 1));
...
///?????????????????  figure out how to shut them down before going on!!
		...
		while(step>0 && list1.getTotalNumofPossible() != 0)
		{
	    	...
	    	do {
				...
				//futures.add(
				executor1.submit(new callTaskDivision(indexThread, groups1.get(indexThread), list1, inputFile, arrTime));
		...
		{
	    	groupDivision subTest = new groupDivision(elements, classElements);
			...
			do {
				...
				//futures.add(
				executor2.submit(new callTaskDivision(indexThread, groups2.get(indexThread), list1, inputFile, arrTime));
				//if(timer.get() > time2)
				//{ time2 = timer.get(); }
